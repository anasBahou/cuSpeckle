...
__global__ void compute_intensity_kernel_float(curandStatePhilox4_32_10_t *state,
                                        int samples,
                                        unsigned int *result, int x, int y, Random_disk* disk_arr, int rc_size, float sigma)
{
 ...
 * @return int 
*/
int monte_carlo_estimation_cuda(float *speckle_matrix, float *Random_centers, float *Random_radius, float *RBound, int number, unsigned int seed, int width, int height, float sigma, float alpha, int nbit, float gamma, int N0)
{
    ...
    for (int x = 1; x < width + 1; ++x)
    {
        for (int y = 1; y < height + 1; ++y)
        {
            ...
            compute_intensity_kernel_float<<<grid, block, block.x *sizeof(unsigned int)>>>(devPHILOXStates, N0, devResults, x, y, disk_arr, count, sigma);
            ...
            else
            {
                ...
                compute_intensity_kernel_float<<<grid, block, block.x *sizeof(unsigned int)>>>(devPHILOXStates, NMC, devResults, x, y, disk_arr, count, sigma);
                /* Copy device memory to host */
                CUDA_CALL(cudaMemcpy(hostResults, devResults, grid.x *
                    sizeof(unsigned int), cudaMemcpyDeviceToHost));
